...
def Paras_VL_Bayes_sum1_Vsoftmax_TN_2to1():
    para = def_parameters()
    para['which_TN_set'] = 'BayesTN'
    para['TN'] = 'VL_Bayes_sum1_Vsoftmax_TN_2to1'
    para['batch_size'] = 2000
    para['d'] = 2
    para['chi'] = 2
    para['feature_map'] = 'cos_sin'
...
class VL_Bayes_sum1_Vsoftmax_TN_2to1_BP(TTN_basic):
    """
    All non-TI pool
    High learnability; overfitting
                     train_acc   test_acc
    ...
    """
    def __init__(self, para_tn, tensors=None):
        super(VL_Bayes_sum1_Vsoftmax_TN_2to1_BP, self).__init__(num_layers=11)
        theta = 1
        self.f_map = para_tn['feature_map']
        add_bias = False
        pre_process_tensors = 'square'   # 'normalize', 'softmax', 'square'
        ...
        self.layer0 = Vectorization(
            para_tn['d'], para_tn['feature_map'], para_tn['device'], theta_m=theta)
        self.layer1 = TTN_Pool_2xto1(
            1, 1, 14, 28, para_tn['d'], para_tn['chi'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.layer2 = TTN_Pool_2yto1(
            1, 1, 14, 14, para_tn['chi'], para_tn['chi'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.layer3 = TTN_Pool_2xto1(
            1, 1, 7, 14, para_tn['chi'], para_tn['chi'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.layer4 = TTN_Pool_2yto1(
            1, 1, 7, 7, para_tn['chi'], para_tn['chi'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.layer5 = TTN_Pool_2xto1(
            1, 1, 4, 7, para_tn['chi'], para_tn['chi'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.layer6 = TTN_Pool_2yto1(
            1, 1, 4, 4, para_tn['chi'], para_tn['chi'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.layer7 = TTN_Pool_2xto1(
            1, 1, 2, 4, para_tn['chi'], para_tn['chi'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.layer8 = TTN_Pool_2yto1(
            1, 1, 2, 2, para_tn['chi'], para_tn['chi'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.layer9 = TTN_Pool_2xto1(
            1, 1, 1, 2, para_tn['chi'], para_tn['chi'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.layer10 = TTN_Pool_2yto1(
            1, 1, 1, 1, para_tn['chi'], para_tn['channel'],
            para_tn['device'], ini_way=para_tn['mps_init'],
            if_pre_proc_T=pre_process_tensors, add_bias=add_bias)
        self.input_tensors(tensors)
